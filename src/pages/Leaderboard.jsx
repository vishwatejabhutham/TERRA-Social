  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      try {
        ...
        console.warn("API call failed, falling back to mock leaderboard data:", error);
      } finally {
        setLoading(false);
      }
    };
      ...
      {leaderboardData.length >= 3 && (
        <div className="grid grid-cols-3 gap-4 md:gap-8 items-end mb-12 mt-16 px-4">
          ...
          <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="flex flex-col items-center">
            ...
            <Card className="w-full bg-gradient-to-t from-teal-50 to-white border-teal-200 text-center pt-8 pb-4 relative -mt-10 rounded-t-xl rounded-b-none h-32 md:h-40 flex flex-col justify-end">
          ...
          <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="flex flex-col items-center">
            ...
            <Card className="w-full bg-gradient-to-t from-lime-50 to-white border-lime-200 text-center pt-8 pb-4 relative -mt-10 rounded-t-xl rounded-b-none h-28 md:h-36 flex flex-col justify-end">
        ...
        <ul className="divide-y divide-gray-100">
          {leaderboardData.slice(3).map((user, idx) => (
            <motion.li 
              ...
              className={`p-4 sm:p-6 flex items-center justify-between transition-colors hover:bg-white/80 ${user.isCurrentUser ? 'bg-gradient-to-r from-emerald-100 to-white border-l-4 border-emerald-500' : ''}`}
            >
              ...
              <div className="text-right">
                <div className="font-bold border border-emerald-200 bg-emerald-100 text-emerald-800 px-3 py-1 rounded-lg inline-flex items-center gap-1 text-sm shadow-sm">
